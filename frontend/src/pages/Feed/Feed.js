...
const Feed = () => {
  ...
  useEffect(() => {
    fetch("https://twitter-qgxu.onrender.com/posts/posts")
      .then((res) => res.json())
      .then((data) => {
        setPosts(data);
      });
  ...
  useEffect(() => {
    fetch(`https://twitter-qgxu.onrender.com/auth/userStatus?email=${email}`)
      .then((res) => res.json())
      .then((data) => {
  ...
  const oneMonth = async () => {
    try {
      if (pts >= 200 && !isSubscribed) {
        ...
        const response = await fetch(`https://twitter-qgxu.onrender.com/payment/convert?email=${email}&pts=${200}`, {
          method: 'PATCH'
        });
        if (!response.ok) {
  ...
  const oneYear = async () => {
    try {
      if (pts >= 2000 && !isSubscribed) {
        // update the points  method = patch
        const response = await fetch(`https://twitter-qgxu.onrender.com/payment/convert?email=${email}&pts=${2000}`, {
          method: 'PATCH'
        });
        if (!response.ok) {
          throw new Error('Network response was not ok');
